...
config.gpu_options.per_process_gpu_memory_fraction = 1.0  # 指定显存分配比例
session = tf.Session(config=config)
...
tf.flags.DEFINE_string('X', 'RS_train/fog.tfrecords',
                       'X tfrecords file for training, default: data/tfrecords/apple.tfrecords')
tf.flags.DEFINE_string('Y', 'RS_train/unfog.tfrecords',
                       'Y tfrecords file for training, default: data/tfrecords/orange.tfrecords')
tf.flags.DEFINE_string('Y_smooth', 'RS_train/unfog_smooth.tfrecords',
                       'Y tfrecords file for training, default: data/tfrecords/orange.tfrecords')
...
def train():
    if FLAGS.load_model is not None:
        ...
        checkpoints_dir = "RS_train/checkpoints"
        try:
    ...
    with graph.as_default():
        cycle_gan = CycleGAN(
            ...
            Y_smooth_train_file=FLAGS.Y_smooth,
    ...
    with tf.Session(config=config, graph=graph) as sess:
        if FLAGS.load_model is not None:
            ...
            meta_graph_path = str(checkpoint) + '.meta'
            print(tf.train.latest_checkpoint(meta_graph_path))
            restore = tf.train.import_meta_graph(meta_graph_path)
            restore.restore(sess, checkpoint)
            step = 130000
        else:
        ...
        try:
            ...
            while not coord.should_stop():
                ...
                if step % 10 == 0:
                    ...
                    # subprocess.call(r"./create_model.sh")
                step += 1
