...
tf.flags.DEFINE_string('checkpoint_dir',r'checkpoints-densenet/110000', 'checkpoints directory path')
tf.flags.DEFINE_string('XtoY_model', 'fog2unfog-110000.pb', 'XtoY model name, default: apple2orange.pb')
tf.flags.DEFINE_string('YtoX_model', 'unfog2fog-110000.pb', 'YtoX model name, default: orange2apple.pb')
...
def export_graph(model_name, XtoY=True):
  ...
  with graph.as_default():
    ...
    input_image = tf.placeholder(tf.float32, shape=[FLAGS.image_size1, FLAGS.image_size2, 3], name='input_image')
    cycle_gan.model()
    if XtoY:
  ...
  with tf.Session(graph=graph) as sess:
    ...
    tf.train.write_graph(output_graph_def, 'pretrained-densenet', model_name, as_text=False)
