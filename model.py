...
class CycleGAN:
  def __init__(self,
    ...
    self.Y_smooth_train_file = Y_smooth_train_file
  ...
  def model(self):
    ...
    Y_reader = Reader(self.Y_train_file, name='Y',
        image_size1=self.image_size1, image_size2=self.image_size2, batch_size=self.batch_size)
    Y_smooth_reader = Reader(self.Y_smooth_train_file, name='Y_smooth',
                      image_size1=self.image_size1, image_size2=self.image_size2, batch_size=self.batch_size)

    x = X_reader.feed()
    y = Y_reader.feed()
    y_smooth = Y_smooth_reader.feed()
    ...
    D_Y_loss = self.discriminator_loss_Y(self.D_Y, y,self.fake_y,y_smooth,use_lsgan=self.use_lsgan)

    # Y -> X
    fake_x = self.F(y)
    F_gan_loss = self.generator_loss(self.D_X, fake_x, use_lsgan=self.use_lsgan)
    F_loss = F_gan_loss + cycle_loss + perceptual_loss #+ pixel_loss
    D_X_loss = self.discriminator_loss_X(self.D_X, x,self.fake_x,y_smooth, use_lsgan=self.use_lsgan)
  ...
  def discriminator_loss_Y(self, D, y, fake_y,fake_xy_smooth, use_lsgan=True):
    ...
    if use_lsgan:
      ...
      c = D(y)
      error_real = tf.reduce_mean(tf.squared_difference(c, REAL_LABEL))
      error_fake_y = tf.reduce_mean(tf.square(D(fake_y)))
      error_fake_y_smooth = tf.reduce_mean(tf.square(D(fake_xy_smooth)))
    else:
      # use cross entropy
      error_real = -tf.reduce_mean(ops.safe_log(D(y)))
      error_fake_y = -tf.reduce_mean(ops.safe_log(1-D(fake_y)))
      error_fake_y_smooth = -tf.reduce_mean(ops.safe_log(1-D(fake_xy_smooth)))
    loss = (error_real + error_fake_y + error_fake_y_smooth) / 3
    return loss

  def discriminator_loss_X(self, D, y, fake_y, real_yx_smooth,use_lsgan=True):
    """ Note: default: D(y).shape == (batch_size,5,5,1),
                       fake_buffer_size=50, batch_size=1
    Args:
      G: generator object
    ...
    if use_lsgan:
      ...
      error_real_y_smooth = tf.reduce_mean(tf.squared_difference(D(real_yx_smooth),REAL_LABEL))
    else:
      # use cross entropy
      error_real = -tf.reduce_mean(ops.safe_log(D(y)))
      error_fake = -tf.reduce_mean(ops.safe_log(1-D(fake_y)))
      error_real_y_smooth = -tf.reduce_mean(ops.safe_log(D(real_yx_smooth)))
    loss = (error_real + error_fake + error_real_y_smooth) / 3
    return loss
